...
@YoneTelethonClient.on(
    events.NewMessage(
        pattern="/sudoinfo ", from_users=(TIGERS or []) + (DRAGONS or []) + (DEMONS or [])
    )
)
...
def gifid(update: Update, context: CallbackContext):
    ...
    if msg.reply_to_message and msg.reply_to_message.animation:
        ...
        update.effective_message.reply_text("Please reply to a gif to get its ID nibba.")
...
def info(update: Update, context: CallbackContext):
    ...
    text += f"\nPermalink: {mention_html(user.id, '{}')}"
    ...
    if user.id == OWNER_ID:
        ...
        text += "\n\nThe Disaster level of this person is 'mates'."
        disaster_level_present = True

    if disaster_level_present:
        text += ' [<a href="https://t.me/komisanupdates/4">?</a>]'.format(
            bot.username
        )
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id == bot.id and sender_id not in DEV_USERS:
            message.reply_text(
                "Erm... yeah, I only trust Darling and Sensei  to set my bio."
            )
            return
...
*What is that health thingy?*
 Come and see [HP System explained](https://t.me/komisanupdates/5)
"""
